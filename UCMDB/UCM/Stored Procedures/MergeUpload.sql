...
BEGIN
	...
	-- Handle things differently for linked sections since we can't have cascading

	MERGE INTO UniScraper.UCM.linked_section WITH (HOLDLOCK) AS TARGET
	USING (SELECT id FROM #class) AS SOURCE(id)
	ON TARGET.parent = SOURCE.id
	WHEN NOT MATCHED BY SOURCE THEN
	DELETE;

	MERGE INTO UniScraper.UCM.linked_section WITH (HOLDLOCK) AS TARGET
	USING (SELECT id FROM #class) AS SOURCE(id)
	ON TARGET.child = SOURCE.id
	WHEN NOT MATCHED BY SOURCE THEN
	DELETE;
    ...
    WHEN NOT MATCHED BY TARGET THEN
	INSERT (class_id, professor_email) VALUES (SOURCE.class_id, SOURCE.professor_email)
	WHEN NOT MATCHED BY SOURCE THEN
	DELETE; 

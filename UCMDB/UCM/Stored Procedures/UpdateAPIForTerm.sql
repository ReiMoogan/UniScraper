CREATE PROCEDURE [UCM].[UpdateAPIForTerm]
    @term INT
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN TRAN
        SELECT course_reference_number AS crn INTO #crn FROM [UCM].[class] WHERE term = @term;
        DELETE FROM [UCM].[v1api] WHERE term = @term;
        ...
        INTO #linked_section
        FROM #crn
        INNER JOIN [UCM].[linked_section] ON #crn.crn = [linked_section].parent;
        ...
        INTO #professor
        FROM #crn
                 INNER JOIN [UCM].[class] ON class.course_reference_number = #crn.crn AND term = @term
        ...
        INTO #class
        FROM #crn
                 INNER JOIN [UCM].[class] ON class.course_reference_number = #crn.crn AND class.term = @term
                 LEFT JOIN [UCM].[subject] ON subject.subject = SUBSTRING(class.course_number, 0, CHARINDEX('-', class.course_number));

        -- reimu
        INSERT INTO [UCM].[v1api]
        SELECT
            #class.crn,
            subject,
            course_id,
